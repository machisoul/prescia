...
for stock_symbol in all_stock_symbol:
    if stock_symbol.startswith("SZ30") or stock_symbol.startswith("SH688") or stock_symbol.startswith("BJ"):
        continue
    history_url = FilterURL(base_history_url, symbol=stock_symbol, current_unix_time=str(int(time.time() * 1000)), count="-20")
    ...
    for index, item in enumerate(items):
        ...
        # if True:
        if (continue_drop_count >= 5) and (close > last_close) and (close > open_value) and (turnoverrate > 1.5) and (continue_drop_value > 0.2):
            continue_drop_count = 0
