...
for stock_symbol in all_stock_symbol:
    if stock_symbol.startswith("SZ30") or stock_symbol.startswith("SH688") or stock_symbol.startswith("BJ"):
        continue
    history_url = FilterURL(base_history_url, symbol=stock_symbol, current_unix_time=str(int(time.time() * 1000)), count="-365")
    ...
    for index, item in enumerate(items):
        ...
        # if True:
        if (continue_drop_count >= 3) and (close > last_close) and (close > open_value) and (turnoverrate > 2.0) and (continue_drop_value > 0.15):
            continue_drop_count = 0
            ...
            for i in range(index + 1, end_index):
                ...
                if (next_high > close) and ((next_high - close) / close > 0.045):
                    # next_utc_time = datetime.datetime.utcfromtimestamp(float(next_timestamp)/1000 + 86400).strftime("%Y-%m-%d %H:%M:%S.%f")
